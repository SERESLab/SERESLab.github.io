...
// Define the task sequence for 2x2 grids
const TASK_SEQUENCE = [
  // Happy emotion tasks
  ...
  if (completed) {
    return (
      <div style={styles.completionContainer}>
        <p style={styles.completionText}>2x2 Grid Face Recognition Task Completed!</p>
      </div>
    );
  }
      ...
      {!showCross && !showInstruction && currentTask && (
        <div className="grid" style={gridStyle}>
          {grid.map((image, idx) => {
            ...
            const aoName = `face-task-2x2-${taskIdx}-trial-${trialNum}-row${rowStr}-col${colStr}-${correctness}`;
            return (
              <img
